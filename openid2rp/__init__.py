...
def _cp_validate(session, return_url, provider_url, query):
    if "assoc_handle" not in (session.get("oid") or {}):
        return _cp_authenticate(session, return_url, provider_url)
    signed = authenticate(session["oid"], query)
...
def _cp_authenticate(session, return_url, provider_url):
    try:
        if provider_url is None:
            raise TypeError()
        services, url, _ = discover(provider_url)
    except (TypeError, ValueError):
        pass
    else:
        session["oid"] = s = associate(services, url)
        return request_authentication(services, url,
            s["assoc_handle"], return_url + "?returned")
    raise ValueError("url")

def _do_cp_login(query, session, return_url, provider_url):
    if "returned" in query:
        return _cp_validate(session, return_url, provider_url, query)
    return _cp_authenticate(session, return_url, provider_url)

def cp_login(query={}, provider=""):
    import cherrypy
    url = ({
    ...
    url = url if url is not None else provider
    try:
        url = _do_cp_login(query, cherrypy.session, cherrypy.url(), url)
    except:
        cherrypy.session.pop("login", None)
        cherrypy.session.pop("oid", None)
        raise
    if url:
        raise cherrypy.HTTPRedirect(url, 307)
    else:
        cherrypy.session.pop("oid", {})
        return cherrypy.session["login"]

def email(session):
    if session:
        for k in ("openid.ax.value.email", "openid.sreg.email",
                "openid.ext1.value.email"):
            if k in session and (session.get("signed") and
                    k in session["signed"].split(",")):
                return session[k]
        for k in ("openid.ax.value.email", "openid.sreg.email",
                "openid.ext1.value.email"):
            if k in session:
...
def name(session):
    if session:
        s = {k.split(".")[-1].lower(): v for k, v in session.items()
            if k.startswith("openid.sreg")
            or k.startswith("openid.ax.value")
            or k.startswith("openid.ext1.value")}
        if s.get("fullname") or s.get("nickname"):
            return s.get("fullname") or s.get("nickname")
        if s.get("first") and s.get("last"):
            return s.get("first") + " " + s.get("last")
        if s.get("firstname") and s.get("lastname"):
            return s.get("firstname") + " " +  s.get("lastname")
        if s.get("first") or s.get("last"):
            return s.get("first") or s.get("last")
        if s.get("firstname") or s.get("lastname"):
            return s.get("firstname") or s.get("lastname")
        e = email(session)
        if e and "@" in e:
            return e.split("@")[0]
